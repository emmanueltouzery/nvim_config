...
function _G.quick_set_ft()
  local filetypes = {
    "typescript", "json", "elixir", "rust", "lua", "diff", "sh", "markdown", "html", "config",
    "sql", "strace", "xml", "other", "logcat", "astgrep", "csv",
  }
...
vim.keymap.set("n", "<leader>pp", function() vim.pack.update(nil, { target = 'lockfile', force = true }) end, { desc = "Sync packages"})
...
vim.keymap.set("n", "<leader>om", ":messages<cr>", {desc="Open messages"})
...
vim.keymap.set("n", "<leader>od", function() require('apidocs').apidocs_open({
  ensure_installed = { "lodash~4", "javascript", "date_fns", "react", "openjdk~8", "rust", "dom", "jq", "lua~5.1"}
...
vim.keymap.set("n", "<leader>tH", function()
  if vim.w.custom_highlight then
    ...
    vim.ui.input({prompt="Enter pattern to highlight", kind="center_win", default=vim.g.custom_highlight_val or ""}, function(pattern)
      ...
      vim.w.custom_highlight = true
      vim.w.custom_highlight_val = pattern
...
function telescope_commits_mappings(prompt_bufnr, map)
  ...
  map('i', '<C-r>r', function(nr)
    commit = require("telescope.actions.state").get_selected_entry(prompt_bufnr).value
    vim.ui.select({"Yes", "No"}, {prompt="Revert the commit " .. commit .. "?"}, function(choice)
      if choice == "Yes" then
...
function telescope_branches_mappings(prompt_bufnr, map)
  ...
  map('i', '<C-Del>', function(nr) -- delete
    ...
    current_picker:delete_selection(function(selection)
      ...
      if string.match(branch, "^origin/") then
        ...
        -- local branch
        vim.system({'git', 'branch', '-D', branch }, {text = true},
  ...
  map('i', '<M-a>', function(nr) -- search in added compared to branch
    local branch = require("telescope.actions.state").get_selected_entry(prompt_bufnr).value
...
vim.keymap.set('n', '<leader>cch', function()
  vim.cmd(":1sp | wincmd k | norm 1G | wincmd j")
